...
def rnn_step_backward(dnext_h, cache):
    ...
    # prev_h of shape (N, H).
    # Note:  the order of the dot matters, particularly for dWh
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    (N, T, D) = x.shape
    (N, H) = h0.shape
    # iterate over the T dimension
    h = np.zeros((N, T, H))
    cache = {} # dictionary indexed by the T dimension
    for t in range(T):
        if t == 0:
            # the first step
            h[:, t, :], cache[t] = rnn_step_forward(
                x[:, t, :], h0, Wx, Wh, b)
        else:
            # use the previous state information for next state
            h[:, t, :], cache[t] = rnn_step_forward(
                x[:, t, :], h[:, t-1, :], Wx, Wh, b)
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def word_embedding_forward(x, W):
    ...
    out = array([W[x[i], :] for i in range(N)])
    cache = (x, W)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def word_embedding_backward(dout, cache):
    ...
    # HINT: Look up the function np.add.at                                       #
    #  http://nullege.com/codes/search/numpy.add.at
    ##############################################################################
    # (N, T, D) = dout.shape
    (x, W) = cache
    (N, T) = x.shape
    ...
    dW = np.zeros((V, D))
    # accumulate the gradients over each word of each sample
    for n in range(N): # iterate each sample
        for t in range(T):  # iterate over each word in a sample
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # calculate the activation vector which consists of the four gates within LSTM:
    #   input gate, forget gate, output gate and block gate
    # A of shape (N, 4H)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros((N, T, H))
    prev_h = h0                 # initial hidden state
    ...
    for t in range(T):
        # use the previous state information for next state
        h[:, t, :], next_c, cache[t] = lstm_step_forward(
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    (x, prev_h, prev_c, Wx, Wh, b, gates, next_c) = cache[0]
    (N, D) = x.shape
    (N, T, H) = dh.shape
    ...
    for t in reversed(range(T)):
        ...
        (dx[:, t, :], dprev_h, dprev_c, dWx_t, dWh_t, db_t) = \
            lstm_step_backward(dh[:, t, :] + dprev_h, prev_c, cache[t])
